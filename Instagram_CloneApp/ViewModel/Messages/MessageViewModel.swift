...
class MessageViewModel : ObservableObject{
    ...
    func sendMessage(message:String){
        ...
        Firestore.firestore().collection("messages").document(userId).collection("user-messages")
            .document(currentUserID).collection("messages").addDocument(data: data) { err in
                if let err = err{
                    print(err.localizedDescription)
                    return
                }
                Firestore.firestore().collection("messages")
                    .document(self.currentUserID).collection("user-messages").document(userId).collection("messages").addDocument(data: data) { err in
                        if let err = err{
                            print(err.localizedDescription)
                            return
                        }
